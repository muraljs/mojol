...
module.exports = (collectionName) => {
  ...
  const middlewares = {
    create: [(ctx, next) => {
      return db.db[collectionName].save(ctx.args)
        .then((doc) => { ctx.res = doc })
        .then(() => next())
    }],
    read: [(ctx, next) => {
      return db.db[collectionName].findOne(ctx.args)
        .then((doc) => { ctx.res = doc })
        .then(() => next())
    }],
    update: [(ctx, next) => {
    ...
    delete: [(ctx, next) => {
      return db.db[collectionName].remove(ctx.args)
        .then((doc) => { ctx.res = doc })
        .then(() => next())
    }],
    list: [(ctx, next) => {
      return db.db[collectionName].find(ctx.args)
        .then((doc) => { ctx.res = doc })
        .then(() => next())
    }]
  }
