...
module.exports = (collectionName) => {
  ...
  const middlewares = {
    create: [async (ctx, next) => {
      ctx.res = await collection(collectionName).save(ctx.args)
      next()
    }],
    read: [async (ctx, next) => {
      ctx.res = await collection(collectionName).findOne(ctx.args)
      next()
    }],
    ...
    delete: [async (ctx, next) => {
      ctx.res = await collection(collectionName).remove(ctx.args)
      next()
    }],
    list: [async (ctx, next) => {
      ctx.res = await collection(collectionName).find(ctx.args)
      next()
    }]
  }
